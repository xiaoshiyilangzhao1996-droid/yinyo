...
"一个面向飞书 + DeepSeek 工作流的 harness Agent。"
...
YINYO 是一个对标 Hermes 和 OpenClaw 的聚焦型 harness Agent 产品，不是通用聊天机器人包装层。它以飞书和 DeepSeek 作为第一产品落地面，把运行时网关、DeepSeek 优先模型网关、TemporalTree 记忆、Trace2Skill 进化、证据记录和发布门禁放进同一条可部署产品线。
...
GitHub 用户现在可以用 `v1.0.0-lite` 连接真实飞书应用测试。按
[docs/external-testing.md](docs/external-testing.md) 从 clone 或安装开始，
配置飞书自建应用，启动长连接服务，收集脱敏 smoke evidence，并在不泄露密钥的前提下共享
`smoke-bundle`。
发布 GitHub Release 时，使用 [RELEASE_NOTES.md](RELEASE_NOTES.md) 作为
`v1.0.0-lite` 的发布正文和资产清单。

外部 live 报告可以进入 `1.0.0` 评审，但只有严格候选门禁带 verified ws bundle 通过后，
才能发布 `v1.0.0`：
...
| 核心 | 含义 |
|---|---|
| Less is more | 飞书优先，小而可审计的工具面，不做平台蔓延。 |
| Borrow what works | 记忆、上下文、演进等研究启发机制必须落到可测试行为。 |
| DeepSeek adapted | 大上下文、低成本调用、工具调用、重试/fallback 和用量遥测是一等设计假设。 |

YINYO 还保持六个行为特质：好奇心、靠谱、事实洁癖、多元化思维、能忍受不确定性、低 ego 高自驱。Release matrix 会把每个核心和特质映射到本地可执行证据；`1.0.0` 还要求真实飞书证据证明同一组产品 claim。
...
| 能力 | 产品路径 |
|---|---|
| Harness 运行时 | 飞书长连接传输、HTTP fallback、事件校验、幂等、任务队列、发信边界和 smoke 证据。 |
| DeepSeek 优先执行 | Provider chain、用量统计、重试、fallback 元数据和运行 manifest 成本估算。 |
| 可演进记忆 | TemporalTree 通过 supersession 维护事实演进，避免旧事实无限堆积。 |
| 自我改进 | Trace2Skill 从重复失败中提取技能，生成回归夹具，重放通过后才 promotion。 |
| 证据卫生 | 工具调用、阻断动作、脱敏 smoke 记录和发布检查全部落盘。 |
| 3+6 证据矩阵 | 场景回放把产品核心和行为特质映射到可执行检查。 |

---

## 运行方式

默认产品传输是飞书长连接 `ws`。HTTP webhook 保留为 fallback 和本地诊断路径。
...
| Smoke 场景 | 必需 |
|---|---:|
| URL verification | 仅 HTTP fallback |
| Text message reply | 是 |
| Image message reply | 是 |
| Card fallback | 是 |
| Duplicate callback | 是 |
...
YINYO 聚焦飞书和 DeepSeek 中心的 Agent 工作流，不追求成为全平台通用 Agent gateway。
...
`1.0.0` 门禁要求 smoke 记录必须由匹配的 runtime logs、durable job records、event idempotency records，以及本地 JSONL stores 使用的 single-writer runtime lock 背书。
...
# 保持服务运行，先完成 text/image/card-fallback/duplicate 飞书 live 动作，再记录 advanced evidence。
...
发布元数据提升也有门禁：`prepare_release_metadata.py --apply` 在 `1.0.0` 下必须带 `--verified-bundle <dir>`，并拒绝未验证或非 ws bundle。
...
`yinyo smoke bundle` 会从 `yinyo.env` inherits `ws_sdk_session_id`；如果传入 `--ws-sdk-session-id`，它 must match config 里的值。
它还会从 config 里的 `app_id` 计算 `feishu_app_id_hash = sha256(app_id)`；如果传入 `--feishu-app-id-hash`，它 must match `sha256(app_id)`。

live `card_fallback` smoke 场景需要临时在 `yinyo.env` 设置 `smoke_mode=true`，并向机器人发送 `/yinyo-smoke card-fallback`；之后关闭 smoke mode、重启，再收集其它 live 场景并构建最终 bundle。
