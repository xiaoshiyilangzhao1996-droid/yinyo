...
**范围：** 核心运行时 ~650 行 Python。总计 ~1,400 行（含 VectorCache 语义检索 + Plan 阶段）。
...
yinyo/
├── __init__.py              ←  15 行
├── agent.py                 ← 200 行  (纯 ReAct Agent Loop)
├── context.py               ← 200 行  (Context Manager + 自动触发)
├── evidence.py              ← 200 行  (Evidence Ledger + 三态 Verification)
├── memory.py                ← 120 行  (5层 + SHADOW 实体 + SimpleMem 集成)
├── model.py                 ← 150 行  (模型调用 + tools 参数 + Context Caching)
├── governance.py            ← 100 行  (Risk + Secret + Gate)
├── tools.py                 ← 180 行  (7 原子工具 + edit + patch + evidence 管线)
├── evolution.py             ←  80 行  (技能结晶 + SelfCheck 自动触发 + 5事件)
─────────────                ────
总计                         ~1,200 行
